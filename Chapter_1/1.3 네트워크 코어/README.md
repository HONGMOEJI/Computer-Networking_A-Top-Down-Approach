...
- `멀티홈(multi-homing)`
    ...
    - 즉, 멀티홈이 중요한 가장 큰 이유는 **가용성(availability)** 때문이다.
      하나의 ISP에만 의존하면 그 ISP의 장애나 회선 문제만으로도 외부 인터넷과의 연결이 끊길 수 있지만,  
      둘 이상의 ISP에 연결되어 있으면 다른 경로로 우회할 수 있다.
    - 또한 상황에 따라 더 나은 경로를 선택할 수 있으므로 **지연 감소, 혼잡 완화, 성능 향상**에도 도움이 된다.
    - 사업적인 관점에서도 특정 제공자에게 완전히 종속되지 않으므로  
      **비용 협상력 확보와 트래픽 운영의 유연성**을 높일 수 있다.
...
최종 정리하자면, `1.3 네트워크 코어`는 **"데이터가 네트워크 안에서 어떻게 전달되고, 그 네트워크들이 모여 오늘날의 인터넷을 어떻게 이루는가"**를 설명하는 장이다.

> 💡 한 줄로 요약하면, 오늘날의 인터넷은 **패킷 교환 방식**을 사용하는 거대한 `네트워크의 네트워크`이며, 라우터들이 패킷을 목적지까지 **홉(hop) 단위로 전달**하고, 수많은 ISP와 콘텐츠 사업자 네트워크가 계층 구조와 직접 연결(피어링)로 서로 얽혀 있다.

### 1. 데이터 전달 방식의 핵심

- 네트워크 코어에서 데이터를 옮기는 기본 방식은 `패킷 교환`과 `회선 교환` 두 가지다.
- `패킷 교환`에서는 큰 메시지를 작은 `패킷`으로 나누어 보내며, 각 패킷은 링크와 패킷 스위치(라우터 등)를 거쳐 이동한다.
- 대부분의 패킷 스위치는 `저장-후-전달(store-and-forward)` 방식을 사용하므로, **패킷 전체를 받은 뒤에야 다음 링크로 전송**할 수 있다.
- 링크 전송률이 `R`, 패킷 길이가 `L`이면 한 링크에서의 전송 시간은 `L/R`이다.
- 출발지와 목적지 사이에 `N`개의 링크가 있으면, 전파/처리/큐잉 지연을 무시할 때 한 패킷의 종단 간 전송 지연은 대략 `NL/R`이다.

### 2. 패킷 지연과 라우터의 역할

- 실제 네트워크에서는 저장-후-전달 지연 외에도 `큐잉 지연`이 생긴다.
- 어떤 출력 링크가 이미 바쁘면 패킷은 `출력 버퍼`에서 기다려야 하고, 이 대기 시간이 바로 큐잉 지연이다.
- 버퍼는 유한하므로 큐가 가득 차면 `패킷 손실(packet loss)`이 발생한다.
- 라우터는 패킷 헤더의 `목적지 IP 주소`를 보고, `포워딩 테이블`을 참조해 어느 출력 링크로 보낼지 결정한다.
- 이 포워딩 테이블은 인터넷의 `라우팅 프로토콜`이 자동으로 구성한다.

### 3. 패킷 교환과 회선 교환의 차이

- `회선 교환`은 통신 전에 종단 간 연결을 설정하고, 통신 동안 필요한 링크 자원을 **미리 예약**한다.
- 그래서 회선 교환은 `보장된 전송률`을 제공할 수 있지만, 사용자가 잠깐 쉬는 동안에도 예약된 자원이 놀게 되어 비효율적일 수 있다.
- 회선 교환의 대표적인 다중화 방식은 `FDM`(주파수 분할 다중화)과 `TDM`(시분할 다중화)이다.
- 반면 `패킷 교환`은 **필요할 때만 링크 자원을 사용**하므로, 사용자 트래픽이 들쭉날쭉한 현실의 인터넷 환경에서 훨씬 효율적이다.
- 대신 패킷 교환은 큐잉 지연 때문에 지연이 가변적이며, 엄격한 실시간 보장은 어렵다.
- 그럼에도 자원 활용도와 확장성 면에서 유리하기 때문에 오늘날 인터넷의 기본 방식은 패킷 교환이다.

### 4. 오늘날 인터넷 구조의 핵심

- 인터넷은 하나의 단일 네트워크가 아니라, 수많은 ISP와 기업 네트워크가 연결된 `네트워크의 네트워크`이다.
- 사용자와 서버는 `접속 ISP(access ISP)`를 통해 인터넷에 들어온다.
- 전체 구조는 대체로 `접속 ISP → 지역 ISP → 1계층(Tier-1) ISP` 형태의 다중 계층 구조를 가진다.
- 하위 ISP는 상위 ISP에 요금을 지불하며 연결성을 얻고, 이를 `고객-제공자(customer-provider)` 관계라고 볼 수 있다.
- `PoP`는 고객 네트워크가 제공자 ISP에 접속하는 실제 접속 지점이다.
- `멀티홈(multi-homing)`은 둘 이상의 제공자 ISP에 연결하는 것으로, 가용성과 성능, 협상력을 높여 준다.
- `피어링(peering)`은 비슷한 계층의 네트워크가 상위 ISP를 거치지 않고 **직접 트래픽을 교환**하는 것이다.
- `IXP`는 여러 네트워크가 한곳에 모여 피어링할 수 있게 해 주는 교환 지점이다.
- 최근에는 구글 같은 대형 `콘텐츠 제공자 네트워크`도 자체 백본망을 구축해 하위 ISP들과 직접 연결하고, 가능한 경우 상위 계층 ISP를 우회한다.

### 5. 시험 직전용 압축 정리

- `패킷 교환` : 패킷 단위 전송, 자원 비예약, 효율적이지만 지연은 가변적
- `회선 교환` : 자원 예약, 전송률 보장, 예측 가능하지만 비효율이 생길 수 있음
- `라우터` : 목적지 IP를 보고 포워딩 테이블에 따라 다음 링크로 전달
- `큐잉 지연/손실` : 링크가 바쁘고 버퍼가 부족할 때 발생
- `인터넷 구조` : 접속 ISP, 지역 ISP, 1계층 ISP, 그리고 PoP, 멀티홈, 피어링, IXP, 콘텐츠 제공자 네트워크로 구성

> 따라서 1.3의 핵심은 **"인터넷은 패킷 교환 기반의 계층적이고도 상호 직접 연결된 네트워크들의 집합이며, 라우터와 ISP들이 협력해 패킷을 목적지까지 전달한다"**는 점이다.
