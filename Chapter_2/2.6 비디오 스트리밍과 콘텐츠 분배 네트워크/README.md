...
비디오의 중요한 특징은 압축될 수 있다는 것인데, 일반적으로 **비디오 품질과 비트 전송률은 비례 관계**를 가진다.

오늘날의 상용 압축 알고리즘은 근본적으로 원하는 여러 비트 전송률에 맞추어 비디오를 압축할 수 있다.  
즉, **비트 전송률이 높을수록 더 많은 영상 정보를 담을 수 있어 일반적으로 화질이 좋아지고**,  
비트 전송률이 낮을수록 더 많은 정보를 버려야 하므로 화질이 낮아질 수 있다.
...
반대로, **압축을 더 강하게 할수록** 보통 비트 전송률은 낮아지지만 품질은 떨어질 수 있다.
...
쉽게 말해, **고용량 비디오 스트리밍 자체가 HTTP/2의 등장으로 처음 가능해진 것은 아니다.**

비디오 스트리밍이 가능해진 배경에는 다음 요소들이 함께 작용한다.

- 비디오 압축 기술의 발전
- 네트워크 대역폭의 향상
- 사용자 가까이에 콘텐츠를 배치하는 CDN
- 재생 중 끊김을 줄이기 위한 버퍼링
- HTTP 기반의 안정적인 파일/조각 전송

여기에 `DASH`가 더해지면서,  
이미 가능하던 비디오 스트리밍을 **현재 네트워크 상황에 맞게 품질을 조절하며 더 안정적으로 재생**할 수 있게 되었다고 이해하면 된다.

즉:

- `HTTP/2`는 웹 전송 효율을 높이는 기술
- `DASH`는 네트워크 상황에 따라 비디오 품질을 적응적으로 바꾸는 기술

으로 역할을 구분해서 보는 것이 좋다.
