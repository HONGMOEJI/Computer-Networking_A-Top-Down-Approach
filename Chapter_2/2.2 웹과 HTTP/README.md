...
<details>
<summary><b>RFC 1945, RFC 7230, RFC 7540 간단히 보기</b></summary>
...
- `HTTP/1.0`를 설명하는 문서다.
- 비지속 연결 중심의 초기 HTTP 동작을 이해할 때 중요하다.
- 요청 하나를 보내고 응답 하나를 받은 뒤 연결을 닫는 방식과 잘 맞는다.

즉, 책에서 설명하는 `HTTP/1.0`의 기본 동작을 볼 때 참고할 수 있는 RFC다.

### RFC 7230

- `HTTP/1.1`의 **메시지 문법(message syntax)** 과 **라우팅(routing)** 을 다룬다.
- 요청 라인, 응답 라인, 헤더, 메시지 본문이 어떻게 생겼는지 이해할 때 중요하다.
- 지속 연결, Host 헤더, 메시지 형식 같은 내용을 이해하는 데 직접 연결된다.

즉, 우리가 흔히 보는 `GET /index.html HTTP/1.1` 같은 메시지 구조는 이 계열 문서와 연결된다.

### RFC 7540

- `HTTP/2`를 정의하는 문서다.
- 핵심은 텍스트 기반 메시지를 그대로 흘리는 대신, **프레임 기반(binary framing)** 으로 바꾸고 하나의 TCP 연결에서 여러 요청/응답을 더 효율적으로 처리하는 데 있다.
- 멀티플렉싱, 헤더 압축, 서버 푸시 같은 HTTP/2 특징을 이해할 때 중요하다.

즉, `HTTP/1.1`과 달리 왜 `HTTP/2`가 더 효율적으로 여러 요청을 처리할 수 있는지를 설명하는 RFC라고 보면 된다.

### 한 번에 정리

- `RFC 1945` -> `HTTP/1.0`
- `RFC 7230` -> `HTTP/1.1`의 메시지 형식과 라우팅
- `RFC 7540` -> `HTTP/2`
...
<p align="center"><img width="760" alt="HTTP Stateless Protocol" src="./stateless-http.svg"></p>

<br/>

위 그림처럼 같은 클라이언트가 같은 객체를 다시 요청해도,  
서버는 "이전에 이미 보냈던 요청"이라는 상태를 따로 기억하지 않고 **각 요청을 독립적으로 처리**한다.
...
이는 서버에서 열고 유지해야 하는 소켓 수를 줄일 뿐만 아니라,  
여러 개의 병렬 TCP 연결을 억지로 열지 않고도 더 효율적으로 데이터를 전송하기 위한 것이다.

하지만 웹 페이지 전체를 오직 하나의 TCP 연결로만 보내게 되면,  
큰 응답 하나가 연결을 오래 점유하여 작은 객체들이 뒤에서 기다리는 `HOL(Head-of-Line) 블로킹` 문제가 생길 수 있다.

HTTP/2는 이를 줄이기 위해 `프레이밍(framing)`이라는 메커니즘을 사용한다.

<br/>

> 💡 `HTTP/2 프레이밍(framing)`이란, HTTP 메시지를 작은 `프레임(frame)` 단위로 쪼갠 뒤, 여러 메시지의 프레임을 하나의 TCP 연결 위에서 섞어서(interleaving) 보내고, 반대편에서 다시 원래 메시지로 조립하는 방식이다.

예를 들어, 아주 큰 비디오 응답 1개와 크기가 작은 객체 8개가 있다고 하자.

HTTP/1.1에서는 큰 비디오 응답이 먼저 길게 전송되면, 뒤에 있는 작은 객체들이 오래 기다릴 수 있다.

하지만 HTTP/2에서는:

1. 큰 비디오 응답을 많은 프레임으로 나눈다.
2. 작은 객체들도 각각 몇 개의 프레임으로 나눈다.
3. 서버는 비디오 프레임만 연속으로 보내지 않고,  
   `비디오 프레임 1 -> 작은 객체 A 프레임 1 -> 작은 객체 B 프레임 1 -> 비디오 프레임 2 ...`  
   처럼 여러 프레임을 번갈아 섞어서 보낸다.

이렇게 하면 작은 객체들이 큰 비디오 응답이 끝날 때까지 계속 기다리지 않아도 되므로,  
애플리케이션 계층에서의 HOL 블로킹을 줄일 수 있다.

<p align="center">
  <img width="520" alt="HTTP/2 multiplexing requests and responses in a single TCP connection" src="https://mdn.github.io/shared-assets/images/diagrams/http/messages/http-2-connection.png">
</p>

<p align="center">
  <sub>Source: <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Messages#http2_messages">MDN Web Docs - HTTP messages</a></sub>
</p>

<br/>
<br/>

**즉, HTTP/2의 가장 중요한 개선점은 HTTP 메시지를 통째로 순서대로 보내는 것이 아니라, 작은 프레임으로 잘라 여러 요청/응답을 한 연결에서 동시에 흘려보낼 수 있게 만든 데 있다.**

프레이밍은 HTTP/2 내부의 프레이밍 서브 계층에 의해 이루어진다.

서버가 HTTP 응답을 보내고자 할 때, 응답은 이 프레이밍 서브 계층에 의해 여러 프레임으로 나뉜다.

예를 들어, 응답 헤더는 헤더 프레임으로, 응답 본문은 여러 데이터 프레임으로 쪼개질 수 있다.

이 프레임들은 다른 응답의 프레임들과 인터리빙된 뒤, 하나의 지속적인 TCP 연결 위로 전송된다.

클라이언트는 도착한 프레임들을 다시 원래의 HTTP 메시지 단위로 재조립한 뒤 브라우저가 처리한다.  
(클라이언트에서 서버로 요청을 보낼 때도 같은 원리다.)

<br/>

또한 프레이밍 서브 계층은 프레임을 **바이너리 형식**으로 인코딩한다.

바이너리 프로토콜은 일반적으로 텍스트 기반 형식보다 파싱이 효율적이고, 더 정교하게 전송 구조를 다룰 수 있다.
