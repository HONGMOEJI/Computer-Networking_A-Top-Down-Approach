...
직렬화는 **메모리 안의 객체를 네트워크로 보낼 수 있는 바이트 형태로 바꾸는 과정**이다.

예를 들어 Python 객체인:

```python
HelloRequest(name="moeji")
```

는 그대로는 네트워크 선로를 통해 보낼 수 없다.  
그래서 gRPC/protobuf는 이 객체를 **정해진 바이너리 형식의 바이트열**로 바꿔서 전송한다.

그리고 수신 측에서는 그 바이트열을 다시:

```python
HelloRequest(name="moeji")
```

같은 객체로 복원하는데, 이 과정을 `역직렬화(deserialization)`라고 한다.
...
다른 말로 하면, 직렬화는 **프로그램이 이해하는 자료구조를 네트워크가 보낼 수 있는 형태로 변환하는 과정**이다.

## 왜 gRPC를 쓰는가?

gRPC를 쓰는 가장 큰 이유는 **네트워크 통신을 바이트 단위가 아니라 함수 호출처럼 다루기 위해서**다.

직접 소켓으로 구현하면 개발자가 보통 아래를 직접 처리해야 한다.
...
- 바이트를 문자열/객체로 변환
- 어디까지가 한 메시지인지 구분
- 클라이언트와 서버가 같은 규약을 지키도록 관리

반면 gRPC를 쓰면:

- `.proto`로 요청/응답 구조를 정의하고
- 코드 생성기로 클라이언트/서버 코드를 만들고
- `stub.SayHello(...)`처럼 함수를 호출하듯 사용하면 된다.

즉, 개발자는 **"무슨 바이트를 어떤 형식으로 보낼까?"** 보다  
**"어떤 요청 객체를 보내고 어떤 응답 객체를 받을까?"** 에 집중할 수 있다.

## "직접 소켓 send/recv를 다루지 않는다"는 뜻

이 말은 **개발자가 네트워크 바이트 스트림을 직접 조립하고 파싱하지 않아도 된다**는 뜻이다.

소켓을 직접 쓸 때는 예를 들어 이런 걸 개발자가 정해야 한다.

- `"name=moeji"` 같은 문자열로 보낼지
- JSON으로 보낼지
- 메시지 길이를 앞에 붙일지
- 응답 형식을 어떻게 맞출지

하지만 gRPC는 이런 일을 프레임워크가 대신 처리한다.
...
- 소켓 방식: `send(b"...")`, `recv(4096)`
- gRPC 방식: `stub.SayHello(request)`

처럼 훨씬 높은 수준에서 코드를 작성할 수 있다.

한 줄로 정리하면, **소켓은 바이트를 주고받는 도구이고, gRPC는 그 위에서 정의된 요청/응답을 함수 호출처럼 다루게 해주는 프레임워크**다.
