...
쉽게 말하면 DNS 메시지는 **"무엇을 물어보는지"와 "그 답이 무엇인지"를 담는 봉투**다.
...
- `응답(reply)` 메시지
  - "IP 주소는 이것이야"
  - "내가 직접 답은 모르지만, 다음에는 이 DNS 서버에게 물어봐"

즉, DNS 요청과 응답은 같은 큰 형식을 공유하지만,  
**요청에서는 질문 영역이 중요하고 응답에서는 답변/책임/추가 영역이 중요**해진다.

예를 들어 브라우저가 `www.example.com`의 IP 주소를 알고 싶다면, DNS 질의 메시지는 대략:

- 질문: "`www.example.com`에 대한 `A` 레코드를 달라"

는 의미를 담고 있다.

그리고 응답 메시지는 대략:

- 답변: "`www.example.com`의 IP 주소는 `93.184.216.34`"

또는

- 책임/추가 정보: "최종 답은 없지만, 다음에는 이 DNS 서버에 물어봐"
...
- 이 메시지가 **어떤 질의/응답에 대응하는지** 식별
- 이 메시지가 **질의인지 응답인지** 구분
- 재귀 지원 여부 같은 **동작 방식** 표시
- 뒤에 이어지는 각 영역에 **레코드가 몇 개 들어있는지** 표시
...
- `답변 영역`
  - "최종 답이 있다면 무엇인가?"
- `책임 영역`
  - "이 이름에 대해 책임 있는 DNS 서버는 누구인가?"
- `추가 영역`
  - "바로 다음 단계에 도움이 되는 부가 정보는 무엇인가?"

<br/>

### 왜 추가 영역이 중요한가?

추가 영역은 DNS 질의 횟수를 줄여주는 데 매우 중요하다.

예를 들어 `example.com`의 `MX` 레코드를 질의했다고 해보자.
...
- 먼저 메일 서버 이름을 알고
- 이어서 그 서버 IP 주소까지 한 번에 얻을 수 있다.

즉, 추가 영역은 **"다음에 또 물어봐야 할 정보를 미리 같이 넣어주는 영역"** 이라고 이해하면 된다.

<br/>

### 한 번에 보는 DNS 메시지 해석

예를 들어 `www.example.com`의 A 레코드를 묻는 DNS 질의를 생각해보자.
...
- `답변 영역`
  - 비어 있을 수 있음
- `책임 영역`
  - `example.com`을 담당하는 DNS 서버 정보
- `추가 영역`
  - 그 DNS 서버의 IP 주소 정보

즉, DNS 메시지는 단순히 "IP 하나를 돌려주는 포맷"이 아니라,  
**질문, 답변, 위임(delegate), 다음 단계에 필요한 힌트까지 함께 담을 수 있는 구조**라고 이해하면 된다.
