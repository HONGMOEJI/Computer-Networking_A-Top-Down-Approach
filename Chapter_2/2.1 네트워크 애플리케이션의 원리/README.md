...
### 참고: IPC(Inter-Process Communication) 방식들

프로세스 간 통신(IPC)에는 여러 방식이 있지만, 이 장의 네트워크 애플리케이션 문맥에서는 **소켓**이 가장 중요하다.

- `파이프(pipe)` : 주로 같은 시스템 안에서 프로세스끼리 바이트 스트림을 주고받음
- `메시지 큐(message queue)` : 메시지 단위로 데이터를 주고받음
- `공유 메모리(shared memory)` : 여러 프로세스가 같은 메모리 영역을 공유함
- `시그널(signal)` : 간단한 이벤트를 알리는 용도
- `소켓(socket)` : 같은 호스트 내부에서도 쓸 수 있고, **다른 호스트의 프로세스와도 통신 가능**

즉, IPC는 넓은 개념이고 소켓은 그중 하나다.  
하지만 **네트워크 애플리케이션은 다른 종단 시스템의 프로세스와 통신해야 하므로**, 이 책에서는 IPC 일반론보다 `소켓`을 중심으로 설명한다.
